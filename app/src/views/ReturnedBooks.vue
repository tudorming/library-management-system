<template>
  <div v-if="$root.$isLoggedIn" class="books">
    <h1 style="text-align: center;">This is the books to return page</h1>
    <h2 style="text-align: center;">Here the employees/admin can set the status of the orders to 'RETURNED'</h2>
      ...
      >
        <template v-slot:top>
          <v-toolbar
            ...
            color="third"
          >
            <v-toolbar-title style="color: #DAD7CD" class="third">Select Order to change status to RETURNED</v-toolbar-title>
            <v-divider
              class="mx-4"
              inset
              vertical
  ...
  export default {
    ...
    methods: {
      initialize () {
        ...
        axios.get('http://localhost:3008/api/get-orders-lent'+urlSearch)
          .then(response => {
      ...
      returnBooks() {
        ...
        let books = this.selectedOrders[0].orderedbooks
        axios.get('http://localhost:3008/api/return-books?id=' + id + '&books='+books)
          .then(response => {
            if (response.data.results) {
              this.initialize()
              this.snackbarConfirm = true
              this.snackbarText = "Changed the status of the order to RETURNED"
              // console.log(this.books[0].front);
              // console.log("books gathered")
            } else if (response.data.results == []){
              this.snackbarError = true
