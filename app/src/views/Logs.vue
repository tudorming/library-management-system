<template>
  <div v-if="$root.$isLoggedIn" class="books">
    <h1 style="text-align: center;">This is the page where you can see all the orders</h1>
    <h2 style="text-align: center;">Here the employees/admin can view the status of all orders</h2>
      ...
      >
        <template v-slot:top>
          <v-toolbar
            ...
            color="third"
          >
            <v-toolbar-title style="color: #DAD7CD" class="third">View all orders</v-toolbar-title>
            <v-divider
              class="mx-4"
              inset
              vertical
  ...
  export default {
    ...
    methods: {
      initialize () {
        ...
        axios.get('http://localhost:3008/api/get-orders-all'+urlSearch)
          .then(response => {
      ...
      //   axios.get('http://localhost:3008/api/return-books?id=' + id)
      //     .then(response => {
      //       if (response.data.results) {
      //         this.initialize()
